...
export const GET: APIRoute = async (context) => {
  ...
  const cleanRuc = cleanIdentifier(ruc || '', 'ruc10-names');
  ...
  const validation = validateAndDeductSearch(context.cookies, 'ruc10-names', provider);
  ...
  const result = await fetchGovData('ruc10-names', cleanRuc, validation.provider);
