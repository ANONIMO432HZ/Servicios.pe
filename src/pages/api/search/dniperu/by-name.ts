...
export const GET: APIRoute = async ({ url, cookies }) => {
  ...
  const apellidoPaterno = url.searchParams.get('apellido_paterno');
  const apellidoMaterno = url.searchParams.get('apellido_materno');

  if (!nombres?.trim() || !apellidoPaterno?.trim() || !apellidoMaterno?.trim()) {
    return new Response(
      JSON.stringify({ success: false, message: 'Requerido: nombres, apellido_paterno y apellido_materno' }),
  ...
  try {
    const result = await ApiDniperu.consultarDniPorNombre(nombres.trim(), apellidoPaterno.trim(), apellidoMaterno.trim());
