...
export const GET: APIRoute = async (context) => {
  ...
  const cleanRuc = cleanIdentifier(ruc || '', 'ruc');
  ...
  const validation = validateAndDeductSearch(context.cookies, 'ruc', provider);
  ...
  const result = await fetchGovData('ruc', cleanRuc, validation.provider);
